...
int main(int argc, char** argv) {
    ...
    int w1, h1, c1;
    u8* orig = stbi_load(orig_filename, &w1, &h1, &c1, 3); // force RGB
    ...
    int w2, h2, c2;
    u8* dist = stbi_load(dist_filename, &w2, &h2, &c2, 3); // force RGB
    ...
    if (!stbi_write_png(error_filename, w, h, 4, error_map, w)) {
